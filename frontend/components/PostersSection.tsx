        ...
        <section className="relative" ref={scrollRef}>
            ...
            <div className="relative md:h-screen flex flex-col-reverse md:flex-row justify-around align-start gap-8 md:gap-20 px-8 md:px-20 py-16">
                ...
                <div className="flex flex-col gap-8 shrink">
                    ...
                    <p className="grow overflow-auto font-light">{activePoster.description}</p>
                    <div className="flex gap-4 max-w-full overflow-auto">
                        {activePoster.images.map((image, index) => (
                            <Image key={index} className="lg:first:hidden" src={image} width={100} height={100} alt={activePoster.title} />
                        ))}
                    </div>
                </div>
                <Swiper className="relative h-auto md:h-full shrink-0 w-full md:w-32" spaceBetween={16} slidesPerView={3} direction="horizontal" grabCursor mousewheel navigation modules={[Mousewheel, Navigation]}>
